...
##################################################################
simRoot <- "/media/dcyr/Data/Sims/LSJ"
#simRoot <- "/media/dcyr/Seagate Backup Plus Drive/Sims/LSJ"
area <- "LSJ"
timesteps <- c(0, 100, 200, 300)  ### can either be 'all' or a vector of user-selected timesteps
...
for (i in seq_along(simDir)) {
    ...
    ### fetching age info
    tsf <- tsfFnc(fireDir = paste(simFolder, "fire", sep = "/"),
                  initTSF = initTSF, timesteps = timesteps)
    tsh <- tshFnc(harvDir = paste(simFolder, "harvest", sep = "/"),
    ...
    save(caribouRS, file = paste0("caribouRS_", a, "_", s, "_", t, "_", r, ".RData"))
    rm("caribouRS")
    removeTmpFiles(h=24)
    t2 <- Sys.time()
    print(t2-t1)
